...
export const editorialMetrics = {
  ...
  "004": { scrollStop: 39.2, retention: 68, completion: null, source: "YouTube Studio H+24" }
};

// Métriques connues par plateforme. Elles complètent les API publiques qui ne
// fournissent pas le scroll-stop / la rétention Studio.
export function applyEditorialMetrics(video = {}) {
  const dossier = normalizeDossier(video);
  if (dossier !== "004") return { ...video, dossier };
  if (String(video.platform || "").includes("YouTube")) {
    return { ...video, dossier, scrollStop: 39.2, retention: 68 };
  }
  if (String(video.platform || "").includes("TikTok")) {
    return { ...video, dossier, completion: 9.4 };
  }
  return { ...video, dossier };
}
