...
async function loadData(token) {
  ...
  return {
    ...
    videos: videosPayload.data?.videos || [],
  };
}
...
export async function GET(request) {
  let accessToken = request.cookies.get("dn_tiktok_access")?.value;
  const refreshTokenValue = request.cookies.get("dn_tiktok_refresh")?.value;
  const expiresAt = Number(request.cookies.get("dn_tiktok_expires")?.value || 0);
  ...
  if (!accessToken && !refreshTokenValue) {
    return NextResponse.json({
      ...
      user: null,
      videos: [],
      message: "TikTok n’est pas connecté.",
    });
  }

  try {
    if ((!accessToken || Date.now() >= expiresAt) && refreshTokenValue) {
      renewedTokens = await refreshToken(refreshTokenValue);
    ...
    if (renewedTokens) setTokenCookies(response, renewedTokens);
    return response;
  } catch (error) {
    const response = NextResponse.json(
      {
        ...
        user: null,
        videos: [],
        message: error instanceof Error ? error.message : "TikTok indisponible.",
      },
      { status: 401 }
    );
